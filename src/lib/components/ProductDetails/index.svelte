<script lang="ts">
	...
	import type { ProductDetails_ProductFragment } from '$lib/server/autogen';

	export let product: ProductDetails_ProductFragment;

	let displayImage = 0;

	const next = () => {
		displayImage = (displayImage + 1) % product.image.length;
	};

	const prev = () => {
		displayImage =
			product.image.length - 1 - ((product.image.length - displayImage) % product.image.length);
	};
</script>

<Breadcrumb breadcrumbList={product.breadcrumbList} />

<section
	data-section-id="1"
	data-share=""
	data-category="product-info"
	data-component-id="b9652106_01_awz"
	class="py-20"
>
		...
		<div class="flex flex-wrap -mx-4 mb-24">
			<div class="w-full md:w-1/2 px-4 mb-8 md:mb-0">
				<div class="relative mb-10" style="height: 564px;">
					<button class="absolute top-1/2 left-0 ml-8 transform translate-1/2" on:click={prev}>
						<svg
							width="10"
							height="18"
							viewBox="0 0 10 18"
					...
					<img
						...
						src={product.image[displayImage].url}
						alt={product.image[displayImage].alternateName}
						data-config-id="auto-img-1"
					/>
					<button class="absolute top-1/2 right-0 mr-8 transform translate-1/2" on:click={next}>
						<svg
							width="10"
							height="18"
							viewBox="0 0 10 18"
				...
				<div class="flex flex-wrap -mx-2">
					{#each product.image as image, index}
						<div class="w-1/2 sm:w-1/4 p-2">
							<button
								class:border-blue-300={displayImage === index}
								class="block border w-full"
								on:click={() => (displayImage = index)}
							>
								<img
			...
			<div class="w-full md:w-1/2 px-4">
				<div class="lg:pl-20">
					<div class="mb-10 pb-10 border-b">
						<span class="text-gray-500" data-config-id="label">{product.brand.name}</span>
						<h2
							class="mt-2 mb-6 max-w-xl text-5xl md:text-6xl font-bold font-heading"
						...
						<p class="inline-block mb-8 text-2xl font-bold font-heading text-blue-300">
							<span data-config-id="price">${product.offers.offers[0].price}</span>
							<span
								class="font-normal text-base text-gray-400 line-through"
								data-config-id="price-old">${product.offers.offers[0].listPrice}</span
							>
						</p>
						<p class="max-w-md text-gray-500" data-config-id="desc">
					...
					<div class="flex mb-12">
						<div class="mr-6">
							<span
								...
								data-config-id="label1">QTY</span
							>
							<div
						...
						<div>
							<span
								class="block mb-4 font-bold font-heading text-gray-400 uppercase"
								data-config-id="label2">Size</span
							>
							<select
					...
					<div class="flex flex-wrap -mx-4 mb-14 items-center">
						<div class="w-full xl:w-2/3 px-4 mb-4 xl:mb-0">
							<a
								...
								href="#"
								data-config-id="primary-action">Add to cart</a
							>
						</div>
					...
					<div class="flex items-center">
						<span class="mr-8 text-gray-500 font-bold font-heading uppercase" data-config-id="share"
							>SHARE IT</span
						>
			...
			<ul class="flex flex-wrap mb-16 border-b-2">
				<li class="w-1/2 md:w-auto">
					<a
						...
						href="#"
						data-config-id="tab1">Description</a
					>
				</li>
				<li class="w-1/2 md:w-auto">
					<a
						...
						href="#"
						data-config-id="tab2">Customer reviews</a
					>
				</li>
				<li class="w-1/2 md:w-auto">
					<a
						class="inline-block py-6 px-10 text-gray-500 font-bold font-heading"
						href="#"
						data-config-id="tab3">Shipping &amp; returns</a
					>
				</li>
				<li class="w-1/2 md:w-auto">
					<a
						class="inline-block py-6 px-10 text-gray-500 font-bold font-heading"
						href="#"
						data-config-id="tab4">Brand</a
					>
				</li>
			</ul>
			<h3 class="mb-8 text-3xl font-bold font-heading text-blue-300" data-config-id="tab-title">
				Summer collection and laoreet get
			</h3>
			<p class="max-w-2xl text-gray-500" data-config-id="tab-desc">
				I had interdum at ante porta, eleifend feugiat nunc. In semper euismod mi a accums lorem
				sad. Morbi at auctor nibh. Aliquam tincidunt placerat mollis. Lorem euismod dignissim, felis
				tortor ollis eros, non ultricies turpis.
			</p>
		</div>
	</div>
</section>
